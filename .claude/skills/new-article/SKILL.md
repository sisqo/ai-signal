---
name: new-article
description: Writes and publishes a new AI Signal blog post end to end — research, write, verify, commit, push, deploy. Two independent choices, both optional in the argument string — a mode ("auto" or "ask") and a topic. No mode given defaults to "auto". No topic given triggers discovery focused on practical tool news — new model releases, agentic coding tools and IDEs, developer-tooling launches, and AI's impact specifically on developers' day-to-day work — finding the most interesting recent story not already covered on the site. Policy/regulation, macro-economic society impact, consumer AI products, and generic company/industry moves are out of scope for this default discovery (an explicitly given topic in any of those areas is still written normally). An optional `--publish YYYY-MM-DD` flag anywhere in the string schedules the article for a future date instead of publishing it today — the piece is still written, committed, and pushed immediately, but stays hidden (404) on the live site until that date. Use whenever the user wants a new article written for this blog (e.g. "/new-article", "/new-article ask", "/new-article the new Gemini release", "/new-article ask the new Gemini release", "/new-article --publish 2026-07-15 the new Gemini release", "scrivi un nuovo articolo", "scrivi un articolo da pubblicare il 15 luglio").
user-invocable: true
...
**No topic given:** run several varied web searches covering the last few days, focused on practical tool news rather than general AI news. Cover: new model releases; agentic coding tools and IDEs; notable developer-tooling launches; and AI's impact specifically on developers' and practitioners' day-to-day work (survey/data-driven pieces like adoption or delegation studies — not macro-economic labor-market analysis aimed at a general audience). Leave out of active discovery: policy and regulation, consumer-facing or mainstream AI products, generic company/industry moves (funding, M&A, valuations, rebrands, org changes), and broader societal-impact stories not specifically about developer work — these make fine articles when a user explicitly asks for one by topic, but don't go looking for them by default. Also skip generic AI business news (stock moves, executive reshuffles) that wouldn't interest either audience. Cross off anything that overlaps with Step 1's map.
