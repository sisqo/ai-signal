...
**Content pipeline:**
- Articles live at `content/articles/<slug>.mdx`. Frontmatter fields: `title`, `slug`, `dek` (one-line teaser), `date`, `tag`.
...
- The homepage (`src/app/page.tsx`) always leads with the most recently *published* article — `articles[0]` after `getPublishedArticles()`'s date-descending sort, no manual flag involved — as a large lead treatment via `FeaturedArticle`, and renders the rest as hairline-divided rows via `ArticleRow` — both in `src/components/ArticleIndexRow.tsx`. This is deliberately dynamic rather than a stored `featured` flag: a flag baked in at write time could go stale the moment a previously-scheduled article's date arrives without a new `/new-article` run to update it, which is exactly the bug this replaced. This list is intentionally not a card grid (see DESIGN.md's anti-references).
...
- **Tags:** reuse an existing tag (`Tools`, `Industry`, `Society`) if the topic reasonably fits; keep the vocabulary small and curated. `Tools` is practitioner-facing coverage of specific tools, frameworks, and comparisons — the default focus of this site, and what `/new-article`'s no-topic discovery actively looks for. `Industry` is company and strategic moves — funding, M&A, research initiatives, org changes. `Society` is policy and regulation, work/societal impact, and consumer-facing AI products — stories whose primary relevance is to a general reader, not just tech-industry watchers. `Industry` and `Society` are both still legitimate tags, but as of 2026-07-08 `/new-article`'s default discovery no longer actively searches for that kind of story (it drifted into covering too much policy/macro-news and not enough practical tool coverage) — they now come up only when a user gives that kind of topic explicitly, not from a bare `/new-article` or `/new-article ask` run. Add a new tag only when a topic genuinely doesn't fit any existing one — don't pre-invent categories for stories that haven't been written yet.
